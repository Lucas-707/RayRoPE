...
mode = 'test'
dataroot = f"/grogu/user/yuwu3/RealEstate10K/metafiles/{mode}"
output_root = f"/grogu/user/yuwu3/RealEstate10K/{mode}"

def try_download(url):
    videoname = url.split("=")[-1]
    
    # Check if video file already exists
    video_path = f"{output_root}/{videoname}"
    if os.path.exists(video_path):
        print(f"[INFO] Video {videoname} already exists, skipping download...")
        return True
    ...
    try:
        ydl_opts = {
            ...
            "outtmpl": f"{output_root}/{videoname}",
            "cookies": f"{output_root}/cookies.txt",
        }
...
def download_and_process(data, mode, output_root):
    ...
    # Check if all sequences for this video are already processed
    all_sequences_exist = True
    for seq_id in range(len(data)):
        seqname = data.list_seqnames[seq_id]
        if not os.path.exists(f"{output_root}/{seqname}"):
            all_sequences_exist = False
            break
        else:
            # Check if all frames exist for this sequence
            expected_frames = len(data.list_list_timestamps[seq_id])
            existing_frames = len(glob.glob(f"{output_root}/{seqname}/*.png"))
            if existing_frames < expected_frames:
                all_sequences_exist = False
                break
    
    if all_sequences_exist:
        print(f"[INFO] All sequences for {videoname} already processed, skipping...")
        return
    ...
    if os.path.exists(f"{output_root}/{videoname}"):
        os.system(f"rm {output_root}/{videoname}")  # remove videos
...
def process(data, seq_id, videoname, output_root):
    seqname = data.list_seqnames[seq_id]
    if not os.path.exists(f"{output_root}/{seqname}"):
        os.makedirs(f"{output_root}/{seqname}")
    else:
    ...
    for idx, str_timestamp in enumerate(list_str_timestamps):
        if os.path.exists(
            ...
            continue
        command = (
            f"ffmpeg -loglevel error -ss {str_timestamp} -i {output_root}/{videoname} -vframes 1 "
...
class DataDownloader:
    def __init__(self, dataroot, output_root, mode="test", failure=False):
        ...
        self.list_data = []
        for vid, txt_file in enumerate(self.list_seqnames):
            ...
            print(f"finished {vid + 1}/{len(self.list_seqnames)}: {youtube_url} with {len(list_timestamps)} timestamps")
    ...
    def run(self):
        ...
        with ThreadPoolExecutor(max_workers=num_workers_video) as executor:
            futures = [
                executor.submit(
    ...
    def regenerate_failed_urls(self):
        seq_ids = set(os.listdir(f"{self.output_root}"))
        failed_urls = []
        for data in self.list_data:
            for idx in range(len(data)):
...
if __name__ == "__main__":
    ...
    DataDownloader(dataroot, output_root, mode, failure=False).run()
    DataDownloader(dataroot, output_root, mode, failure=False).show()
    DataDownloader(dataroot, output_root, mode, failure=False).regenerate_failed_urls()
    DataDownloader(dataroot, output_root, mode, failure=False).regenerate_incomplete_urls()
    print(f"Start running with failure=True")
    DataDownloader(dataroot, output_root, mode, failure=True).run()
