...
def gen_transforms(base_dir, mode):
    """
    ...
    for seq in tqdm(seqs, desc=f"Generating transforms for {mode} dataset"):
        ...
        meta_file = os.path.join(base_dir, "metafiles", mode, seq + ".txt")
        ...
        if missing_img_count > 0:
            print(
                f"Warning: {missing_img_count} out of {len(lines) - 1} images are missing in sequence {seq} for {mode} dataset."
            )
...
if __name__ == "__main__":
    base_dir = "/grogu/user/yuwu3/RealEstate10K"
    # mode = "train"
    # reorg_imgs(base_dir, mode)
    # gen_transforms(base_dir, mode)
